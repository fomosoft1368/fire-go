 ...
 * Gọi sau khi driver đăng nhập thành công
 */
export async function registerPushToken(): Promise<string | null> {
  try {
    ...
    if (finalStatus !== 'granted') {
      console.log('[PushNotification] ❌ Tài xế từ chối quyền thông báo')
      return null
    }
    ...
    if (Platform.OS === 'android') {
      ...
      // Channel riêng cho yêu cầu chuyến xe (ưu tiên cao nhất)
      await Notifications.setNotificationChannelAsync('ride-request', {
        name: 'Yêu cầu chuyến xe',
        importance: Notifications.AndroidImportance.MAX,
    ...
    console.log('[PushNotification] ✅ Driver token:', expoPushToken.substring(0, 40) + '...')
...
/**
 * Gửi Expo Push Token lên backend để lưu vào driver.expoPushToken
 */
async function sendTokenToBackend(token: string): Promise<void> {
  try {
    // Driver dùng key 'token' (khác customer dùng 'authToken')
    ...
    if (response.ok) {
      console.log('[PushNotification] ✅ Driver token đã đăng ký với backend thành công')
    } else {
