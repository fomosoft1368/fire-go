...
export class DeliveryController {
  ...
  async create(@Body() createDeliveryDto: CreateDeliveryDto, @Request() req) {
    console.log(
      '[DeliveryController] ========== CREATE DELIVERY START ==========',
    );
    ...
    console.log(
      '[DeliveryController] DTO:',
      JSON.stringify(createDeliveryDto, null, 2),
    );
    ...
    console.log(
      '[DeliveryController] Pickup coords:',
      delivery.pickupCoordinates,
    );

    // Auto-assign driver asynchronously (don't wait for it)
    console.log(
      '[DeliveryController] Setting up auto-assign timer for 1 second...',
    );
    setTimeout(async () => {
      try {
        console.log('[DeliveryController] ⏰ Auto-assign timer triggered!');
        console.log(
          '[DeliveryController] Calling autoAssignDriver for delivery:',
          delivery._id.toString(),
        );
        const result = await this.deliveryAutoAssignService.autoAssignDriver(
          delivery._id.toString(),
        );
        console.log(
          '[DeliveryController] Auto-assign result:',
    ...
    console.log(
      '[DeliveryController] Returning delivery to client (auto-assign scheduled)',
    );
    return delivery;
  }
  ...
  async update(
    ...
    const customerId =
      (delivery as any).customerId?._id?.toString() ||
      (delivery as any).customerId?.toString();
    const driverId =
      (delivery as any).driverId?._id?.toString() ||
      (delivery as any).driverId?.toString();
  ...
  async getPendingAssignmentRequests(@Request() req) {
    ...
    console.log(
      '[DeliveryController] 🔍 Getting pending delivery assignment requests for driver:',
      driverId,
    );
    const requests =
      await this.deliveryAutoAssignService.getPendingRequestsForDriver(
        driverId,
      );
    console.log(
      '[DeliveryController] 📋 Found',
      requests.length,
      'pending delivery assignment requests',
    );
    return requests;
  }
  ...
  async acceptAssignmentRequest(
    ...
    const driverId = req.user.id || req.user.sub;
    const delivery =
      await this.deliveryAutoAssignService.acceptAssignmentRequest(
        requestId,
        driverId,
      );
      ...
      this.pushService
        .notifyCustomer(
          ...
          NotificationType.DELIVERY_ASSIGNED,
          {
            ...
            deliveryId: (delivery as any)._id?.toString(),
          },
        )
        .catch(() => {});
    }
  ...
  async rejectAssignmentRequest(
    ...
    const driverId = req.user.id || req.user.sub;
    await this.deliveryAutoAssignService.rejectAssignmentRequest(
      requestId,
      driverId,
      reason,
    );
