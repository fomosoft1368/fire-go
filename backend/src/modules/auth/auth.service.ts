...
export class AuthService {
  private otpCache = new Map<
    string,
    { code: string; expiresAt: number; email?: string }
  >();
  private loginOtpCache = new Map<
    string,
    { code: string; expiresAt: number; name?: string; email?: string }
  >();
  ...
  async login(loginDto: LoginDto): Promise<AuthResponseDto> {
    ...
    console.log(
      '📊 All drivers in DB:',
      allDrivers.map((d) => ({ email: d.email, phone: d.phone })),
    );
    ...
    console.log(
      '📋 Driver object keys:',
      driver ? Object.keys(driver.toObject?.() || driver) : 'null',
    );
    ...
    if (driver && driver.password) {
      ...
      console.log(
        '🔍 Checking isSuspended - value:',
        driver.isSuspended,
        'type:',
        typeof driver.isSuspended,
      );
      if (driver.isSuspended === true) {
        console.log('🛑 Driver is suspended, blocking login');
        throw new UnauthorizedException(
          'Tài xế tài khoản của bạn đã bị khóa. Vui lòng liên hệ quản trị viên để được hỗ trợ.',
        );
      }
      ...
      return this.generateTokensForDriver({
    ...
    console.log(
      '❌ Driver not found or no password, trying User collection...',
    );
  ...
  async verifyAccount(
    userId: string,
    role: string,
  ): Promise<{ valid: boolean; role: string; status: string }> {
    if (role === 'driver') {
      const driver = (await this.driverModel
        .findById(userId)
        .select('isSuspended isBlacklisted status deletionRequestedAt')
        .lean()) as any;
      if (!driver) {
        throw new UnauthorizedException(
      ...
      if (driver.isSuspended) {
        throw new ForbiddenException(
          'Tài khoản tài xế của bạn đã bị đình chỉ. Vui lòng liên hệ quản trị viên.',
        );
      }
      if (driver.isBlacklisted) {
        throw new ForbiddenException(
          'Tài khoản tài xế của bạn đã bị khóa vĩnh viễn. Vui lòng liên hệ quản trị viên.',
        );
      }
      ...
      const customer = (await this.customerModel
        .findById(userId)
        .select('isBlacklisted isAccountLocked')
        .lean()) as any;
      if (!customer) {
        throw new UnauthorizedException(
          'Tài khoản không tồn tại hoặc đã bị xóa.',
  ...
  async sendCustomerPhoneOtp(
    ...
    this.otpCache.set(userId.toString(), {
      code: otp,
    ...
    console.log(`\n=======================================\n`);
    console.log(
      `🔥 [ZALO OA] Sắp gửi mã OTP [${otp}] qua Zalo tới số: ${customer.phone}`,
    );
    console.log(`\n=======================================\n`);

    // Gọi thực tế API Zalo
    this.sendZaloOtp(customer.phone, otp).catch((e) =>
      console.error('Lỗi khi gửi Zalo OTP:', e.message),
    );

    return { message: 'Đã gửi mã OTP qua Zalo', expires: 300 }; // 300 giây
  }

  async verifyCustomerPhoneOtp(
    userId: string,
    code: string,
    ...
    if (!entry)
      throw new BadRequestException(
        'Mã OTP không tồn tại hoặc chưa được gửi. Vui lòng thử lại.',
      );
  ...
  async sendDriverRegisterOtp(
    ...
    // Generate 6-digit OTP
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    this.otpCache.set(`driver_${rawPhone}`, {
      code: otp,
      expiresAt: Date.now() + 5 * 60 * 1000,
      email,
    });
    ...
    console.log(`\n=======================================\n`);
    console.log(
      `🔥 [EMAIL OTP] Sắp gửi mã OTP Đăng ký Tài xế [${otp}] tới email: ${email}`,
    );
    console.log(`\n=======================================\n`);
  ...
  async sendCustomerLoginOtp(
    ...
    if (customer) {
      ...
      const currentRealEmail =
        customer.email && !customer.email.includes('@firego.local')
          ? customer.email
          : null;
      ...
      if (currentRealEmail) {
        ...
        if (targetEmail && targetEmail !== currentRealEmail.toLowerCase()) {
          throw new BadRequestException(
            'Email nhập vào không khớp với email đã đăng ký của số điện thoại này.',
          );
        }
        ...
        if (!targetEmail) {
          throw new BadRequestException(
            'Tài khoản của bạn chưa cập nhật Email. Vui lòng nhập Email để nhận OTP.',
          );
        }
      }
    } else {
      ...
      if (!targetEmail) {
        throw new BadRequestException(
          'Vui lòng cung cấp Email để đăng ký tài khoản mới',
        );
      }
    ...
    // Generate 6-digit OTP
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    this.loginOtpCache.set(rawPhone, {
      code: otp,
      expiresAt: Date.now() + 5 * 60 * 1000,
      email: targetEmail,
      name,
    ...
    console.log(`\n=======================================\n`);
    console.log(
      `🔥 [EMAIL LOGIN] Sắp gửi mã OTP Đăng nhập [${otp}] tới email: ${targetEmail}`,
    );
    console.log(`\n=======================================\n`);
  ...
  async verifyCustomerLoginOtp(
    ...
    if (!customer) {
      ...
      const pseudoPassword = await bcrypt.hash(
    ...
    return this.generateTokens({
      ...
      tokenVersion: updatedCustomer.tokenVersion,
      role: 'customer',
    } as any);
  }
  ...
  private async sendEmailOtp(
    ...
    try {
      if (!process.env.MAIL_HOST) {
        console.warn(
          `[EMAIL OTP] Chưa cấu hình SMTP (MAIL_HOST). BỎ QUA GỬI THỰC TẾ, CHỈ IN TERMINAL.`,
        );
        return;
      }
      ...
      throw new BadRequestException(
        'Không thể gửi email OTP, vui lòng kiểm tra lại cấu hình.',
      );
    }
  }
  ...
  private async getZaloAccessToken(): Promise<string> {
    ...
    if (!tokens.refresh_token) {
      throw new Error(
        'Không có refresh_token để lấy access_token từ Zalo. Vui lòng tạo file zalo-token.json',
      );
    }
    ...
    try {
      ...
      if (data.access_token) {
        ...
        console.log(`\n=======================================`);
        console.log(
          `[MOCK MODE KÍCH HOẠT] Vẫn ưu tiên cho App CHẠY CHỨ KHÔNG ĐƯỢC CRASH!`,
        );
        console.log(`=======================================\n`);
  ...
  private async sendZaloOtp(phone: string, otp: string): Promise<void> {
    try {
      ...
      try {
        ...
        console.log(
          '[ZALO ZNS MOCK] Bỏ qua lỗi Zalo. Đang chạy mô phỏng qua Terminal.',
        );
        return;
      }
      ...
      console.log(
        '[FALLBACK TỰ ĐỘNG] Lỗi mạng khi gọi Zalo, chuyển sang gửi SMS...',
      );
  ...
  private async sendZaloOtpPlain(
    ...
    console.log(
      `[ZALO ZNS PLAIN] Đang gửi tới SĐT ${formattedPhone} (Template: ${templateId})...`,
    );
    ...
    if (data.error && data.error !== 0) {
      console.warn(
        `[ZNS PLAIN ERROR ${data.error}] ${data.message || ''} – Fallback SMS...`,
      );
  ...
  private async sendZaloOtpRsa(
    ...
    const znsUrl = 'https://business.openapi.zalo.me/message/template';
    const templateId =
      process.env.ZALO_ZNS_ENCRYPTED_TEMPLATE_ID ||
    ...
    try {
      console.log(
        `[ZALO ZNS RSA] 🔐 Đang mã hóa RSA và gửi tới ${formattedPhone} (Template: ${templateId})...`,
      );

      // Mã hóa từng trường theo yêu cầu Zalo
      const encryptedPhone = ZaloRsaHelper.encrypt(
        formattedPhone,
        rsaPublicKey,
      );
      const encryptedTemplateData = ZaloRsaHelper.encryptTemplateData(
        { otp },
        rsaPublicKey,
      );
      ...
      console.log(
        '[ZALO ZNS RSA] Payload (đã mã hóa):',
        JSON.stringify({
          ...payload,
      ...
      if (data.error && data.error !== 0) {
        console.warn(
          `[ZNS RSA ERROR ${data.error}] ${data.message || ''} – Fallback SMS...`,
        );
        await this.sendSmsOtp(rawPhone, otp);
      } else {
        console.log(
          '[ZALO ZNS RSA] ✅ Gửi OTP RSA thành công! Template được duyệt thẳng.',
        );
      }
    } catch (rsaErr: any) {
  ...
  private async sendSmsOtp(phone: string, otp: string): Promise<void> {
    try {
      ...
      console.error(
        `[TWILIO ERROR] Không thể gửi OTP:`,
        error.message || error,
      );
    }
  }
