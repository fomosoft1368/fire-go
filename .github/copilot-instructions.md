...
- **Backend**: `cd backend && npm run start:dev` → Swagger at `http://192.168.1.9:3000/api/docs`
- **Web Admin**: `cd web-admin && npm run dev` → Vite at `http://192.168.1.9:5173`
- **Mobile**: Expo-managed (`npm run android` / `npm run ios`); Android emulator uses `10.0.2.2` for 192.168.1.9, real devices use `192.168.x.x`
