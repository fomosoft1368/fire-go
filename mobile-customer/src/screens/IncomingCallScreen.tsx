...
const AUTO_DISMISS_MS = 31_000 // 31s (backend timeout 30s + buffer)
...
export default function IncomingCallScreen() {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      console.error('[IncomingCallScreen] Accept error:', err.message)
      navigation.goBack()
    }
  }
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      console.error('[IncomingCallScreen] Reject error:', err.message)
    } finally {
      navigation.goBack()
    }
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.label}>
          {callerRole === 'driver' ? 'Tài xế đang gọi' : 'Khách hàng đang gọi'}
        </Text>
